...
pub struct Config {
    ...
    pub monitor_checks_retention_days: u64,
}
...
pub fn init_config() -> Config {
    ...
    let oauth_issuer_url = std::env::var("OAUTH_ISSUER_URL").expect("OAUTH_ISSUER_URL must be set");
    ...
    let monitor_checks_retention_days = std::env::var("MONITOR_CHECKS_RETENTION_DAYS")
        .unwrap_or_else(|_| "90".to_string())
        .parse()
        .expect("CHECKS_RETENTION_DAYS must be a valid integer");
